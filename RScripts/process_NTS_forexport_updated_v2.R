...
load_NTS = function(path = file.path(parameters$path_secure_data,"National Travel Survey/Safeguarded/")){
  ...
  fls = fls[grepl("(household)|(individual)|(vehicle)|(trip)|(stage)",fls)]
  ...
  for(i in 1:length(fls)){
    sub = foreign::read.spss(file.path(file.path(dir,"UKDA-5340-spss","spss","spss28",fls[i])),
                                  to.data.frame=TRUE)
    sub = sub[sub$SurveyYear %in% years,]
...
nts = load_NTS(path = "C:/Users/malco/OneDrive - University of Leeds/Data/National Travel Survey/Safeguarded")
...
"NumVanLorry","HHIncome2002_B02ID","Ten1_B02ID",
"HHoldGOR_B02ID","HHoldStruct_B02ID")
...
nms_people = c("IndividualID","HouseholdID","PSUID","VehicleID","PersNo",
               "Age_B01ID","Age_B04ID",
               "Sex_B01ID","OfPenAge_B01ID","EthGroupTS_B02ID",
               "NSSec_B03ID",
...
nms_hh[!nms_hh %in% names(hh)]
nms_vehicle[!nms_vehicle %in% names(vehicle)]
nms_people[!nms_people  %in% names(people)]
nms_trips[!nms_trips  %in% names(trips)]
nms_stage[!nms_stage  %in% names(stage)]
...
stage_all = left_join(stage,
                  vehicle[c("VehicleID","CarOwn_B01ID","VehPropTypeTS_B01ID")],
                  by = c("VehicleID"))
